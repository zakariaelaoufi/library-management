	...
	<%
		...
		ArrayList<EmpruntPerdu> listEmpruntPerdu = EmpruntDAO.getAllLatnessEmpruntAfter30Days();
	%>
	...
	<title>Gerer 30j de relance</title>
	...
	<div class="flex h-screen">
		...
		if("valid".equals(msg)) {
		%>
		<div class="mx-auto my-8 bg-amber-100 w-2/3 py-8 rounded-2xl shadow-lg shadow-amber-100/50 flex justify-center items-center">
		<p class="text-xl block text-center text-emerald-950 font-semibold">L'exemplaire est déclaré perdu</p>
		<span class="text-5xl" style='font-size:50px;'>&#128546;</span>
		</div>
		<%}%>
		<% if("allvalid".equals(msg)) { %>
		<div class="mx-auto my-8 bg-amber-100 w-2/3 py-8 rounded-2xl shadow-lg shadow-amber-100/50 flex justify-center items-center">
		    <p class="text-xl text-center text-amber-950 font-semibold mr-2">Tous les exemplaires sont déclarés perdus</p>
		    <span class="text-5xl" style='font-size:50px;'>&#128546;</span>
		</div>
		<%} %>
	    ...
	    <table id="myTable" class="text-[15px] text-left text-gray-900 overflow-hidden rounded-lg">
	        ...
	        <tbody>
	        <% for(EmpruntPerdu emp : listEmpruntPerdu) { %>
	            <tr class="bg-emerald-100 border-b dark:border-white">
	                ...
	                <td class="px-6 py-4"> 
	                	<%=emp.getDate_de_relance() %>
	                </td>
	                <td>
					<a href="./emprunte/considererPerdu?idEmprunte=<%=emp.getId()%>">
						<button  onclick="confirmSubmitForm()" class="w-48 bg-teal-700 border-2 border-teal-700 hover:bg-transparent text-white hover:text-teal-700 font-semibold py-2 px-4 m-2 rounded flex items-center">
						    <p class="mr-1">Consedirer perdu</p>
	...
	function sendRelanceRequest() {
        ...
        if (selectedIds.length === 1) {
            url = './emprunte/considererPerdu?idEmprunte=' + selectedIds[0];
        } else if (selectedIds.length > 1) {
            url = './emprunte/considererPerdu';
    ...
    function confirmSubmitForm() {
        Swal.fire({
            text: "Voulez vous vraiment déclarer l'exemplaire comme perdu?",
            icon: "question",
            showCancelButton: true,
            confirmButtonColor: "#3085d6",
            cancelButtonColor: "#d33",
            confirmButtonText: "Oui, declaré!"
        }).then((result) => {
