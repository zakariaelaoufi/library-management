...
<body>        
		...
		<div class="min-h-screen flex items-center justify-center w-full">
		    <div class="bg-slate-100 shadow-2xl shadow-emerald-200 rounded-lg px-8 py-6 w-1/3">
		        <h1 class="text-2xl font-bold text-center mb-4 text-emerald-800">Espace Assistant</h1>
		        <form id="loginForm" class="form-signin" action="../Login" method="post">
		            ...
		            <button type="submit" name="loginType" value="Assist" class="w-full flex justify-center py-2 px-4 border-2 border-emerald-800 rounded-md shadow-sm text-sm font-medium text-white bg-emerald-800 hover:text-emerald-800 hover:bg-transparent">Connexion</button>
...
document.getElementById('loginForm').addEventListener('submit', function(event) {
  ...
  if (!username.value || !password.value) {
    ...
    if (!username.value) username.classList.add("invalid");
